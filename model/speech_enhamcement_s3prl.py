...
class SpeechEnhancementModel(nn.Module):
    ...
    def __init__(
        ...
        if self.magnitude_head is not None:
            ...
            self.model_head_mag = self.get_head(768 + 201, self.magnitude_head)
    ...
    def get_head(self, input_dim, head_type):
        ...
        if head_type == "lstm":
            ...
            return ConformerHead(
                input_dim=input_dim, hidden_dim=768,
                output_dim=self.stft_embedding_dim, n_heads=12,
    ...
    def forward(self, batch):
        ...
        if self.use_all_layers:
            ...
            wav2vec2_hidden_state = get_s3prl('tera_logMelBase_T_F_M_AdamW_b32_1m_960hr_drop1', batch["input_waveform"])
        ...
        if self.model_head_phase is not None:
            ...
            output_phase = self.model_head_phase(complex_noisy, output_mag_phase) #([32, 798, 201,2])
